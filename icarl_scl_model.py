...
#    optimizer = optim.SGD(net.parameters(), lr=self.LR, weight_decay=self.WEIGHT_DECAY, momentum=self.MOMENTUM)
    ...
    data_train_SCL =  SCLDataset(augmented_dataset.X, augmented_dataset.Y,transform=TwoCropTransform(train_transform))
...
#    loader = DataLoader(data_train_SCL, batch_size=self.BATCH_SIZE,sampler=BalancedBatchSampler(data_train_SCL.X,data_train_SCL.y),drop_last = True)
    loader = DataLoader(data_train_SCL, batch_size=self.BATCH_SIZE,shuffle=True, drop_last = True)
...
#            print(features.shape)
            ...
            if len(self.exemplar_sets) > 0:
                ...
                features1_sim = torch.div(torch.matmul(features1_prev_task, features1_prev_task.T),0.07)
                logits_mask = torch.scatter(torch.ones_like(features1_sim),1,torch.arange(features1_sim.size(0)).view(-1, 1),0)
            ...
            if len(self.exemplar_sets) > 0:
                with torch.no_grad():
                    features2_prev_task = old_net(images)
    
                    features2_sim = torch.div(torch.matmul(features2_prev_task, features2_prev_task.T), 0.07)
    ...
    self.net.fc =  nn.Linear(10,  self.n_classes)
