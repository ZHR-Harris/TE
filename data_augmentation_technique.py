# -*- coding: utf-8 -*-
"""
Created on Tue May 10 21:05:08 2022
...
#        idx = (y_train[400:]== label)
    ...
    train_features_raw = np.load('./X_train_multiclass_shot.npy')
    train_features_raw = torch.from_numpy(train_features_raw).type(torch.FloatTensor)
    train_features_extract = net.encoder(train_features_raw)
    X_train = []
    for feature in train_features_extract:
      feature = feature.squeeze()
      feature.data = feature.data / feature.data.norm() # Normalize
      X_train.append(feature.cpu().detach().numpy())
    
    
#    train_features_extract = train_features_extract.detach().numpy()  
    y_train = np.load('./y_train_multiclass_shot.npy')
    
    test_features_raw = np.load('./X_test_multiclass_shot.npy')
    test_features_raw = torch.from_numpy(test_features_raw).type(torch.FloatTensor)
    test_features_extract = net.encoder(test_features_raw)
    X_test = []
    for feature in train_features_extract:
      feature = feature.squeeze()
      feature.data = feature.data / feature.data.norm() # Normalize
...
#    test_features_extract = test_features_extract.detach().numpy()  
    y_test = np.load('./y_test_multiclass_shot.npy')
    ...
    print(np.sum(y_train==y_predict)/y_test.shape[0])
